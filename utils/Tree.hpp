...
// #######################################################################################
	...
	void _init()
	{
		...
		_alloc_node.construct(_nil, ft::Node<value_type>());
		_nil->_isRed = false;
		_nil->_isNil = true;
		_end = _alloc_node.allocate(1);
		_alloc_node.construct(_end, ft::Node<value_type>());
		_end->_val = _alloc_val.allocate(1);
		_alloc_val.construct(_end->_val, value_type());
		_end->_isRed = false;
	}

	bool _is_nil(node_pointer node) const { return node == _nil || node == _end; }
	...
	node_pointer _copy_node(node_pointer other)
	{
		...
		_alloc_node.construct(tmp, ft::Node<Val>());
	...
	void _balance_after_erase (node_pointer x)
	{
		node_pointer bro;
		...
		while (x != _root && !x->_isRed)
		{
			if (x == x->_parent->_left)
			{
				...
				if (!bro->_left->_isRed && !bro->_right->_isRed)
				{
					bro->_isRed = true;
					x = x->_parent;
				}
				...
				{
					if (!bro->_right->_isRed)
					{
						bro->_left->_isRed = false;
						bro->_isRed = true;
						_rotate_right(bro);
						bro = x->_parent->_right;
					}
					bro->_isRed = x->_parent->_isRed;
					x->_parent->_isRed = false;
					bro->_right->_isRed = false;
					_rotate_left(x->_parent);
					x = _root;
				}
			}
			else
			{
				...
				if (!bro->_left->_isRed && !bro->_left->_isRed)
				{
					bro->_isRed = true;
					x = x->_parent;
				}
...
// #######################################################################################
	...
	Tree(typename ft::enable_if< !ft::is_integral<InputIterator>::value, InputIterator>::type first,
	InputIterator last, const val_compare& comp = val_compare(), const allocator_type& alloc = allocator_type()) :
			_alloc_val(alloc), _alloc_node(node_allocator()), _comp(comp), _root(NULL), _size(0)
	{
		_init();
		_root = _end;
...
// #######################################################################################
	...
	iterator	begin() { return (iterator(_size == 0 ? _end : iterator(_tree_min(_root)))); }
	const_iterator	begin() const { return (const_iterator(_size == 0 ? _end : const_iterator(_tree_min(_root)))); }
...
// #######################################################################################
	...
	ft::pair<iterator, bool> insert (const value_type& val)
	{
		...
		_alloc_node.construct(newNode, ft::Node<value_type>(_create_val(val)));
		newNode->_left = _nil;
		newNode->_right = _nil;
	...
	{
		node_pointer y = pos.iter(), z = y, for_free = y, x;
	...
	val_compare value_comp() const { return (_comp); }
